...
const translations: Record<string, Record<Lang, string>> = {
    ...
    menu_check:             { uz: '🔍 Tekshirish',            en: '🔍 Check' },
    menu_settings:          { uz: '⚙️ Sozlamalar',            en: '⚙️ Settings' },
    menu_fallback:          { uz: '👋 Menudan bo\'limni tanlang yoki /help yuboring.', en: '👋 Select a section from the menu or send /help.' },
    ...
    settings_lang_changed_en: { uz: '✅ Language changed: English', en: '✅ Language changed: English' },

    // ── Login flow ─────────────────────────────────────────────────────────
    login_title:            { uz: '🔒 *Kabinetga kirish*\n\nEmail yoki Consulting nomini kiriting:', en: '🔒 *Cabinet Login*\n\nEnter your email or consulting name:' },
    ...
    connect_first:          { uz: '⚠️ Oldin kabinetni ulang (⚙ Consultingni ulash orqali).', en: '⚠️ Please connect a cabinet first (via ⚙ Settings).' },
    connect_first_slash:    { uz: '⚠️ Kabinet ulanmagan. Avval /cabinet orqali ulaning.', en: '⚠️ Cabinet not connected. Connect first via /cabinet.' },
    ...
    cat_pending:            { uz: 'Kutilmoqda', en: 'Pending' },
    cat_approved:           { uz: 'Tasdiqlandi', en: 'Approved' },
    cat_cancelled:          { uz: 'Rad etildi', en: 'Rejected' },
    cat_application:        { uz: 'Arizalar', en: 'Applications' },
    ...
    check_passport_prompt:  { uz: '🔍 *Tezkor tekshirish*\n\nPasport raqamini kiriting (misol: AA1234567):', en: '🔍 *Quick Check*\n\nEnter passport number (e.g. AA1234567):' },
    check_passport_invalid: { uz: '⚠️ Pasport raqami xato. Misol: AA1234567. Qaytadan kiriting:', en: '⚠️ Invalid passport. Example: AA1234567. Try again:' },
    check_name_prompt:      { uz: '👤 Talabaning *Ism-familiyasi*ni kiriting (inglizcha, pasportdagidek):', en: '👤 Enter student\'s *Full Name* (in English, as in passport):' },
    ...
    check_appno_prompt:     { uz: '📄 E-Visa ariza raqamini kiriting (misol: 6595150001):', en: '📄 Enter E-Visa application number (e.g. 6595150001):' },
    ...
    // ── Autofill ───────────────────────────────────────────────────────────
    autofill_found:         { uz: '🔍 *Ma\'lumot topildi*\n\nPasport *{passport}* bo\'yicha talaba topildi. Bu shu talabami?', en: '🔍 *Record found*\n\nA student was found for passport *{passport}*. Is this them?' },
    ...
    // ── PDF ────────────────────────────────────────────────────────────────
    pdf_loading:            { uz: '⏳ *Sertifikat yuklab olinmoqda...*\n_Iltimos kutib turing, visa.go.kr portaliga so\'rov yuborilmoqda..._', en: '⏳ *Downloading certificate...*\n_Please wait, sending request to visa.go.kr portal..._' },
    pdf_no_student:         { uz: '❌ Talaba ma\'lumotlari topilmadi. Avval statusni tekshiring.', en: '❌ Student data not found. Check status first.' },
    pdf_caption:            { uz: '📄 *Koreya vizasi sertifikati* ({passport})', en: '📄 *Korea Visa Certificate* ({passport})' },
    pdf_error:              { uz: '❌ *Sertifikatni yuklab bo\'lmadi:* {error}', en: '❌ *Could not download certificate:* {error}' },
    btn_pdf:                { uz: '📥 Viza (pdf)', en: '📥 Visa (pdf)' },

    // ── Save to cabinet ────────────────────────────────────────────────────
    save_prompt:            { uz: '💾 Bu talabani kabinetga saqlansinmi?', en: '💾 Save this student to cabinet?' },
    ...
    save_updated:           { uz: '✅ *{passport}* kabinetda yangilandi.', en: '✅ *{passport}* updated in cabinet.' },
    save_restored:          { uz: '✅ *{passport}* kabinetga qayta qo\'shildi.', en: '✅ *{passport}* re-added to cabinet.' },
    save_saved:             { uz: '✅ *{passport}* kabinetga saqlandi!', en: '✅ *{passport}* saved to cabinet!' },
    save_error:             { uz: '❌ Saqlashda xatolik: {error}', en: '❌ Save error: {error}' },

    // ── Keyboards ─────────────────────────────────────────────────────────
    visa_type_embassy:      { uz: 'Elchixona', en: 'Embassy' },
    ...
    notif_partner:          { uz: '🏢 *Hamkor:*', en: '🏢 *Partner:*' },
