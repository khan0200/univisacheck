...
class VisaCalcLeadExtractor {
    ...
    // `latestMessage` is optional -- pass it when `history` doesn't already
    // include the newest turn; omit it (or pass '') when `history` is
    // already the complete transcript (e.g. including the AI's own reply
    // for this turn, which is where the visa-percentage estimate lives).
    async extract(history, latestMessage) {
        const transcript = (latestMessage ? [...history, { role: 'user', content: latestMessage }] : history)
