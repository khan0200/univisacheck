 ...
 * Ported from the legacy lib/auth.ts — signToken/JWT_SECRET redirected to
 * server/utils/auth.ts (the shared Nitro JWT helper), both now async since
 * that helper lazily resolves the secret via dynamic import.
 *
  ...
  try {
    ...
    if (currentSession) {
      try {
        ...
        jwt.verify(currentSession, secret)
