// @ts-nocheck
import IntentAnalyzer from '../lib/ai/intent-analyzer'
import UniversityService from '../lib/university-service'
import KnowledgeProcessor from '../lib/ai/knowledge-processor'
import KnowledgeRetriever from '../lib/ai/knowledge-retriever'
import VisaCalcLeadExtractor from '../lib/ai/visa-calc-lead-extractor'
import VisaCalcLeadService from '../lib/visa-calc-lead-service'
import { loadLocalConfig } from '../lib/local-config'
import { isAdminToken } from '../utils/admin-auth'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (!message) apiError(400, 'Missing message parameter')
    ...
    for (const msg of history) {
      if (msg.role === 'user' && msg.content.startsWith('/login ')) {
        ...
        if (await isAdminToken(token)) isAdmin = true
      }
    }
    ...
    if (isAdmin) {
      if (message.startsWith('/savol ')) {
        ...
        if (result.status === 'success') {
          ...
          return { response: `ℹ️ **Natija:** ${result.message}` }
        }
      }
    }
    ...
    // conversation for no benefit. University entities are still needed
    // though, recovered here via a cheap in-process name match instead of
    // an LLM call.
    ...
    // Fetch top-scholarship universities for "which university gives more
    // scholarship" style questions where no specific university was named.
...
`
    ...
    if (shouldCaptureLead) {
      try {
        // Bounded window instead of full history -- keeps extraction cost
        // flat regardless of how long the conversation has run.
    ...
    const errMessage = err.response?.data?.error ? (err.response.data.error.message || JSON.stringify(err.response.data.error)) : err.message
    console.error('[AI Assistant API Error]:', errMessage)
    apiError(500, 'AI Assistant failed: ' + errMessage)
  }
})
