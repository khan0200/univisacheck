  ...
  >
    <div class="grid lg:grid-cols-[1fr_auto] gap-8 items-center mb-10">
      <div class="flex items-start gap-4">
        ...
        <div>
          <h2 class="text-2xl sm:text-3xl font-bold tracking-tight text-[var(--color-text-primary)] dark:text-white">
            Kerakli hujjatlar
          </h2>
          <p class="text-sm text-[var(--color-text-secondary)] mt-1.5 max-w-md">
            Universitetga qabul va viza olish uchun zarur bo'lgan hujjatlar ro'yxati
          </p>
        </div>
      </div>
      <img
      ...
      >
        <div class="flex items-center justify-between gap-3 px-5 sm:px-6 pt-5">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="font-semibold text-sm text-[var(--color-text-primary)] dark:text-white">
                Qabul uchun hujjatlar
              </h3>
        ...
        <ul class="mt-4 mx-5 sm:mx-6 rounded-xl border border-dashed border-[var(--color-border)] dark:border-white/[0.08] divide-y divide-dashed divide-[var(--color-border)] dark:divide-white/[0.08] overflow-hidden">
          <li
            v-for="doc in admissionPreview"
            :key="doc"
            class="flex items-center gap-2.5 px-3.5 py-2.5 bg-primary-50/30 dark:bg-white/[0.02]"
          >
            <UIcon
              name="i-lucide-file-check-2"
              class="size-3.5 text-primary-600 dark:text-secondary-300 shrink-0"
            />
          ...
          <li class="flex items-center justify-center gap-1.5 px-3.5 py-2.5 bg-primary-50/60 dark:bg-white/5 text-xs font-semibold text-primary-700 dark:text-secondary-300 group-hover:gap-2.5 transition-all">
            Barcha {{ admissionCount }} ta hujjatni ko'rish <UIcon
      ...
      >
        <div class="flex items-center justify-between gap-3 px-5 sm:px-6 pt-5">
          ...
          <UBadge
            ...
            class="shrink-0"
          >
            {{ embassyCount }} tagacha
          </UBadge>
        </div>

        <ul class="mt-4 mx-5 sm:mx-6 rounded-xl border border-dashed border-[var(--color-border)] dark:border-white/[0.08] divide-y divide-dashed divide-[var(--color-border)] dark:divide-white/[0.08] overflow-hidden">
          <li
            v-for="doc in embassyPreview"
            :key="doc"
            class="flex items-center gap-2.5 px-3.5 py-2.5 bg-success-50/30 dark:bg-white/[0.02]"
          >
            <UIcon
              name="i-lucide-file-check-2"
              class="size-3.5 text-success-600 dark:text-success-400 shrink-0"
            />
            <span class="text-xs text-[var(--color-text-primary)] dark:text-white/90 truncate">{{ doc }}</span>
          </li>
          <li class="flex items-center justify-center gap-1.5 px-3.5 py-2.5 bg-success-50/60 dark:bg-white/5 text-xs font-semibold text-success-700 dark:text-success-400 group-hover:gap-2.5 transition-all">
            Barcha hujjatlarni ko'rish <UIcon
              name="i-lucide-arrow-right"
              class="size-3.5"
            />
          </li>
    ...
    <div class="mt-6 flex items-start gap-3 p-4 rounded-xl bg-warning-50 dark:bg-warning-950/30 border border-warning-200 dark:border-warning-900">
      <UIcon
        name="i-lucide-info"
        class="size-4.5 text-warning-600 shrink-0 mt-0.5"
      />
