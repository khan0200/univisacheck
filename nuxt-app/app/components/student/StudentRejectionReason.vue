  ...
  <div v-if="items.length > 0" class="mt-1 text-xs">
    ...
    <div :class="props.compact ? 'space-y-1' : 'space-y-1.5'">
      <div
        ...
        :key="idx"
        class="flex items-center gap-1.5 leading-snug whitespace-nowrap"
      >
        <!-- Red Iconed Number Badge -->
        <span
          v-if="item.number"
          class="inline-flex items-center justify-center size-4.5 min-w-[18px] h-[18px] px-1 rounded-full bg-danger-600 text-white text-[10.5px] font-bold shrink-0 shadow-2xs select-none"
        >
          {{ item.number }}
        </span>
        <UIcon
        ...
        <span class="text-neutral-900 dark:text-neutral-200 font-normal whitespace-nowrap">
          {{ item.text }}
        </span>
      </div>
    </div>
