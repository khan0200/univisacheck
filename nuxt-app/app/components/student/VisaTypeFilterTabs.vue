...
<template>
  <div class="inline-flex items-center gap-1 p-1 rounded-lg bg-neutral-100 dark:bg-white/5 border border-neutral-200/80 dark:border-white/10 w-full sm:w-auto">
    <button
      ...
      :class="props.modelValue === opt.value
        ? 'bg-white dark:bg-primary-900 text-primary-950 dark:text-white shadow-xs font-semibold'
    ...
    >
      ...
      <span>{{ opt.label }}</span>
      <span
        class="text-[10px] font-semibold rounded px-1.5 py-0.2 text-center transition-colors"
        :class="props.modelValue === opt.value
